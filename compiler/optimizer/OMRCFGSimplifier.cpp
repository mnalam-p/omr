...
bool OMR::CFGSimplifier::simplifyBoundCheckWithThrowException(bool needToDuplicateTree) 
   ...
   if (compareNode->getNumChildren() > 1)
      {
      ...
      if (firstChild->getOpCodeValue() == TR::iload)
         ...
         if (secondChild->getOpCodeValue() == TR::iconst && secondChild->getConstValue() == 0)
            ...
            firstChildBndChk = compareNode->getFirstChild();
            }
            ...
            secondChildBndChk = compareNode->getSecondChild();
            }
         }
      }
   ...
   if (secondCompareNode->getNumChildren() > 1) 
      {
      TR::Node *firstChild = secondCompareNode->getFirstChild();
      TR::Node *secondChild = secondCompareNode->getSecondChild();
      ...
      if (firstChild->getOpCodeValue() == TR::iload) 
         ...
         if (secondChild->getOpCodeValue() == TR::iconst && secondChild->getConstValue() == 0) 
            ...
            firstChildBndChk = secondCompareNode->getFirstChild();
            ...
            isLastBlockRetBlock = isSecondCompareLT;
            secondChildBndChk = secondCompareNode->getSecondChild();
            secondNodeRequiresDuplicate = true;
            }
         }
      }

   if (!patternMatched || !firstChildBndChk || !secondChildBndChk || firstChildBndChk == secondChildBndChk || throwBlock != secondThrowBlock) 
      {
      if (trace())
         traceMsg(comp(), "Abort simplifyBoundCheckWithThrowException : %s\n", !patternMatched ? "Pattern did not matched" : 
            ...
            ((firstChildBndChk == secondChildBndChk) ? "Both branch compares either (param,0) or (param,limit)" :
